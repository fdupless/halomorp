...
def numback(E1, E2, numtot, Emin, Emax):  # based off of dN/dE from 1410.3696
    ...
    Int = (
        ...
        * numtot
        / (
            integrate.quad(
                lambda x: np.power(x / Escale, 1 - gam) * np.exp(-x / Ecut), Emin, Emax
            )[0]
        )
    )
    num = (
        Int
        * integrate.quad(
            lambda x: np.power(x / Escale, 1 - gam) * np.exp(-x / Ecut), E1, E2
        )[0]
    )
...
def backgroundevents(
    ns, bmin, ntsr, signalevs, numtot
):  # uses MCsample and outputs an array of background events over allsky
    # signalevs=[signalevs[i]*ntsr for i in range(len(signalevs))] #signalevs has length of Ebins and contains the signal # of events
    num = [
        numback(gv.EBins[j], gv.EBins[j + 1], numtot * ntsr, gv.Emin, gv.Emax)
        for j in range(len(gv.EBins) - 1)
    ]
    eventsback = [
        mcsample(num[i], ns, bmin, 0, 0, 0, 0, 0) for i in range(len(signalevs))
    ]
    return eventsback
...
def SBbackground(numtot, EnBins, ntsr, thcut, thcutHE):  # single Blazar background
    # print(EnBins)
    num = [
        numback(EnBins[j], EnBins[j + 1], numtot * ntsr, gv.Emin, gv.Emax)
        for j in range(len(EnBins) - 1)
    ]
    events = [
        MCsampleSB(num[j], thcut) for j in range(len(EnBins) - 2)
    ]  # LE gamma bins
