...
hel=UI.hel #helicty of the magnetic field for case 4,5 and 6. It takes the value +/- 1
...
blazarloc=np.array([0,0,1])*dS#variable determining the location of the blazar when simulating multiple ones in the sky
x2hat,y2hat,z2hat=np.zeros(3),np.zeros(3),np.array([0,0,1]) #new coord system around blazar
...
def drawMFPg(Egg,zgg):
    if(STOMFPg==0):#these can be changed with gv.STOMFPg=whatever in other specific codes.
        dg=MFPg(zE(dS),Egg)
    else:
