...
def gencascade(Eini=Eini,z=0.24):
    ...
    E=newE(E,z=z)
    while(E>Ecutoff):
        pd.append([de(E,z=z)+pd[-1][0],Eg(E,z=z)+epsilonkek*np.random.uniform()])
        E=newE(E,z=z)
    return np.array(pd)
...
def getdist(Eini=Eini,Nsim=Nsim,nbins=nbins,z=0.24):
    ...
    for i in range(nbins-1):
        tmp=raw[np.where(np.logical_and(lengths>x[i],lengths<x[i+1]))]
        if(len(tmp)>0):
            a,loc,scale=ss.gamma.fit(tmp[:,1],floc=Eg(Ecutoff-0.1))
            emax=np.max(tmp[:,1])
        else:
            loc,scale=0,0
            emax=0
        dist.append([x[i],len(tmp),loc,scale,emax,a])
    dist=np.array(dist)
    dist[:,1]/=np.max(dist[:,1])
    '''
    ...
    for i in range(1,nbins-1):
        ...
        if(len(tmp)>0):
            ...
            dist.append([Eb[i],len(tmp),dist[-1][2],dist[-1][3],dist[-1][4],Xmax]) #if no data, use the last bin's data
    dist=np.array(dist)
...
def dlice(dist,pdf_E,dist2,pdf2,e0,e1,e2):
    ...
    if(Eind!=len(dist) and e0):
        ...
        a1=[LI(eg1,eg2,dist[Eind][2+i],dist[Eind+1][2+i],Egg) for i in range(3)]
        a2=[LI(t2eg1,t2eg2,dist2[Eind][2+i],dist2[Eind+1][2+i],Egg) for i in range(3)]
...
def drawLeptonICevent(dist,pdf_E):
    ...
    '''
    while(logi):
        x=np.random.uniform(0,dist[-1,0]-0.00001)
        y=np.random.uniform()
        xind=len(dist[:,0][dist[:,0]<x])
        if(y<dist[xind,1]):
            tmp=dist[xind,3]*dist[xind,5]*dist[xind,4]
            if(tmp==np.nan or tmp==0):
                logi=True
            else:
                logi=False
                traveldist=x
    ...
    print(dist[xind])
    icounter=0
    while(Egg < gv.Emin or Egg > gv.Emax or icounter<10):
        icounter=icounter+1
        Egg=ss.gamma.rvs(dist[xind,5],loc=dist[xind,2],scale=dist[xind,3])
    print('t2')
    '''
...
def finddist(nbins):
    ...
    Es=np.arange(Ecutoff,Em,Em/100.)
...
def createdistest():
    ...
    t=[test(x/100.) for x in range(100)]
    x=np.arange(100)/100.
    plt.plot(x,t)
    plt.show()
    return
