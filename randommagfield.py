...
def e1(kvec):
    temp = n0(kvec)
    if temp[0] == 1:
        e = np.array([0, -kvec[2], kvec[1]]) / np.sqrt(
            kvec[2] * kvec[2] + kvec[1] * kvec[1]
        )
    else:
        e = np.array([kvec[2], 0, -kvec[0]]) / np.sqrt(
            kvec[2] * kvec[2] + kvec[0] * kvec[0]
        )
    return e
...
def e2(kvec):
    ...
    if temp[0] == 1:
        ...
        e = -np.array([-a, kvec[1] * kvec[0], kvec[0] * kvec[2]]) / np.sqrt(
            kvec[2] * kvec[2] * kvec[2] * kvec[2]
            + kvec[1] * kvec[1] * kvec[1] * kvec[1]
            + 2 * kvec[1] * kvec[2] * kvec[1] * kvec[2]
            + kvec[0] * kvec[2] * kvec[0] * kvec[2]
            + kvec[0] * kvec[1] * kvec[0] * kvec[1]
        )
    else:
        a = kvec[2] * kvec[2] + kvec[0] * kvec[0]
        e = -np.array([kvec[1] * kvec[0], -a, kvec[1] * kvec[2]]) / np.sqrt(
            kvec[2] * kvec[2] * kvec[2] * kvec[2]
            + kvec[0] * kvec[0] * kvec[0] * kvec[0]
            + kvec[1] * kvec[2] * kvec[1] * kvec[2]
            + 2 * kvec[0] * kvec[2] * kvec[0] * kvec[2]
            + kvec[0] * kvec[1] * kvec[0] * kvec[1]
        )
    return e
...
def drawB(k, helicity=0):
    ...
    if sigmap == 0:
        ...
        magBp = np.abs(
            np.random.normal(0, sigmap)
        )  # equation (23) in 1607.00320 -- sort of.

    Breal = np.power(2.0, -0.5) * (
        (magBp * np.cos(phasep) + magBm * np.cos(phasem)) * e1(k)
        + (-magBp * np.sin(phasep) + magBm * np.sin(phasem)) * e2(k)
    )
    Bim = np.power(2.0, -0.5) * (
        (magBp * np.sin(phasep) + magBm * np.sin(phasem)) * e1(k)
        + (magBp * np.cos(phasep) - magBm * np.cos(phasem)) * e2(k)
    )  # equation (25) in 1607.00320
    return Breal + 1j * Bim


def createB(nlattice, Lcutoff, hel=0):
    itok = 2 * np.pi / Lcutoff * (nlattice - 1) / nlattice
    Bx = np.zeros((nlattice, nlattice, nlattice)) + 1j * np.zeros(
        (nlattice, nlattice, nlattice)
    )
    By = np.zeros((nlattice, nlattice, nlattice)) + 1j * np.zeros(
        (nlattice, nlattice, nlattice)
    )
    Bz = np.zeros((nlattice, nlattice, nlattice)) + 1j * np.zeros(
        (nlattice, nlattice, nlattice)
    )
    kvecs = []
    # Setting the norm
        ...
        for j in range(nlattice):
            for k in range(nlattice):
                if i == 0 and j == 0 and k == 0:
                    ...
                    kvec = kint * itok
                    B = (
                        drawB(kvec, helicity=hel)
                        + drawB(np.array([nlattice - i, j, k]) * itok, helicity=hel)
                    ) / 2.0
                    Bx[i, j, k] = B[0]
...
def createB_one_K(knorm, NumModes, hel=0, B0=gv.B0):
    ...
    for i in range(NumModes):
        ...
        kvecs[i] = (
            np.array(
                [
                    np.sin(th[i]) * np.cos(phi[i]),
                    np.sin(th[i]) * np.sin(phi[i]),
                    np.cos(th[i]),
                ]
            )
            * knorm
...
def Banalytic(x1, y1, z1):
    ...
    for i in range(numK):
        ...
        B[0] = (
            B[0]
            + np.real(Bkx[i][0]) * np.cos(xdotk)
            - np.imag(Bkx[i][0]) * np.sin(xdotk)
        )
        B[1] = (
            B[1]
            + np.real(Bky[i][0]) * np.cos(xdotk)
            - np.imag(Bky[i][0]) * np.sin(xdotk)
        )
        B[2] = (
            B[2]
            + np.real(Bkz[i][0]) * np.cos(xdotk)
            - np.imag(Bkz[i][0]) * np.sin(xdotk)
        )
    B[0] = B[0] * 4.0 / numK
    B[1] = B[1] * 4.0 / numK
    B[2] = B[2] * 4.0 / numK
...
def viewB(name, Brms=gv.B0, knorm=1, Nmodes=1, nlattice=30, zslice=0, Bcomp=0):
    ...
    surf = ax.plot_surface(
        X, Y, arB, rstride=1, cstride=1, cmap=cm.RdBu, linewidth=0, antialiased=False
    )
