...
def frhat(t,p):#theta and phi in radians   -- Spherical Radius
    return np.sin(t)*np.cos(p)*xhat+np.sin(t)*np.sin(p)*yhat+np.cos(t)*zhat
def fthat(t,p):#theta and phi in radians  #latitude (azimuthal)
    return np.cos(t)*np.cos(p)*xhat+np.cos(t)*np.sin(p)*yhat-np.sin(t)*zhat
...
def fvf(t,p):#orientation of the GeV gamma ray
    return -frhat(t,p)
def fvavg(t,p,delta):
    return (-zhat)*np.cos(delta/2.0-t)+np.cross(fphat(t,p),zhat)*np.sin(delta/2.0-t)
...
def fB2(x,y,z):
    return yhat*gv.B0
def fB3(x,y,z):
    return (np.cos(gv.beta)*np.cos(gv.beta2)*xhat+np.cos(gv.beta)*np.sin(gv.beta2)*yhat-np.sin(gv.beta)*zhat)*gv.B0
def fB4(x,y,z):
    return (np.cos(2*np.pi*z/gv.cl+gv.dalpha)*yhat+gv.hel*np.sin(2*np.pi*z/gv.cl+gv.dalpha)*xhat)*gv.B0
def fB5(x,y,z):
    return (np.cos(2*np.pi*x/gv.cl+gv.dalpha)*yhat-gv.hel*np.sin(2*np.pi*x/gv.cl+gv.dalpha)*zhat)*gv.B0

if(gv.case==6):
    shat=np.array([np.cos(gv.phi6)*np.sin(gv.theta6),np.sin(gv.theta6)*np.sin(gv.phi6),np.cos(gv.theta6)])
...
def fB6(x,y,z):
    ...
    return (np.cos(2*np.pi*s/gv.cl+gv.dalpha)*sp1+gv.hel*np.sin(2*np.pi*s/gv.cl+gv.dalpha)*sp2)*gv.B0
...
def Binterp(x,y,z):
    ...
    for i in [0,1]:
        for j in [0,1]:
            for k in [0,1]:
                w=3-np.abs(n[0]-nfloor[0]-i)-np.abs(n[1]-nfloor[1]-j)-np.abs(n[2]-nfloor[2]-k)
...
def fdelx(vpara,v,vi,B,e,Egg):#time the electron moves (De/v), angular vel (v/R), speed of el (v,vi) , mag field B, charge e (+/- 1)
    #w2,tau2=w,tau
    if(gv.STODE==1):
    ...
    return v*(vpara*(tau2-np.sin(w2*tau2)/w2)*B+vi*np.sin(w2*tau2)/w2+e*(np.cos(w2*tau2)-1.0)*np.cross(B,vi)/w2)
...
def getxyzB(dg,dE,angles,fullsky=gv.fullsky):#gets the xyz coords from the blazar's frame of reference in order to determine the mag field position.
    if(fullsky):
        temp=dg*np.sin(angles[2]-angles[0])
        vec=gv.blazarloc-dg*np.cos(angles[2]-angles[0])*gv.z2hat+temp*np.cos(angles[1])*gv.x2hat+temp*np.sin(angles[1])*gv.y2hat
...
def fcons2(dg,dE,Egg,angles):
    ...
    return fvcBdphi(fBz(B,angles[0],angles[1]),fBrho(B,angles[0],angles[1]),angles[2],angles[0])
def fcons3(dg,dE,Egg,angles):
    x,y,z=getxyz(dg,dE,angles)
    return fdeltaEqn(angles[2],fvpara(B,angles[0],angles[1],angles[2]),Egg)


def fsolvecsq_dec2016(dg,dE,Egg,tG,pG,deltaG,ranget,Morp=False,xtol=1e-10,tau=False,RnB=False):# Solves the Constraints - a bit faster than the stable one and more likely to find a solution (I think).
    ...
    def F(angles2):
        angles=np.array([np.arcsin(np.sin(angles2[1])*dg/dE),angles2[0],angles2[1]])
        ...
        cons2=fvcBdphi(fBz(fB,angles[0],angles[1]),fBrho(fB,angles[0],angles[1]),angles[2],angles[0])
        cons3=fdeltaEqn(angles[2],fvpara(fB/bnorm,angles[0],angles[1],angles[2]),Egg,wtau)
        return (cons2,cons3)
...
def fsolvecsq_stable(dg,dE,Egg,tG,pG,deltaG,ranget,Morp=False,xtol=1e-10,tau=False,RnB=False):# Solves the Constraints - quite stable, but can jump to other branches
    anglesG=np.array([tG,pG,deltaG])
...
def fsolvecsq(dg,dE,Egg,tG,pG,deltaG,ranget,Morp=False,xtol=1e-10,tau=False,RnB=False):
    return fsolvecsq_dec2016(dg,dE,Egg,tG,pG,deltaG,ranget,Morp=Morp,xtol=xtol,tau=tau,RnB=RnB) #factor of 3 speed increase
