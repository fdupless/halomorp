...
def stomorp(ds,NumPhotons,alpha=0,phi=0,omega=np.pi,LeptonId=False,xtol=1e-10):
    ...
    while(countphoton<NumPhotons):
        ...
        if(gv.jetactivated==1 and np.cos(omega)>cosang):#the second checks that the solution is part of the jet
            injet=False
            countphoton=countphoton+1
        if(ier==1 and np.absolute(sol[0])<np.pi/2.0 and injet):#ier=1 if a solution is found by fsolvecsq
...
def plotStoMorp(figname,phi=0,omega=np.pi,alpha=0,NumEvents=NumEventsDefault,Morp=False,LeptonId=False,blazloc=np.array([0,0,1])*gv.dS,xtol=1e-10):
    res=300
    gv.blazarloc=blazloc #if fullsky is on, this allows us to check at blazar from different locations in a magnetic field.
    
    #import time
    #start=time.clock()
    tcospsol,tsinpsol,events=stomorp(gv.dS,NumEvents,alpha=alpha,phi=phi,omega=omega,LeptonId=LeptonId,xtol=1e-10)
    #print(time.clock()-start)
    Ebins=gv.EBins
    ...
    if(not LeptonId):#color code the events according to the energy bins
        ...
        for i in range(len(events)):
            if(np.floor(events[i][3])==1):
                ax.scatter(tcospsol[i],tsinpsol[i],s=5,marker='^',color=scalarMap.to_rgba(events[i][2]))
            else:
                ax.scatter(tcospsol[i],tsinpsol[i],s=5,marker='v',color=scalarMap.to_rgba(events[i][2]))
        ...
        ax=fig.add_subplot(111)
        lines=[]
        retLine=ax.scatter(scaColorC[0],scaColorS[0],s=5,color='red',marker='^',label='positron')
        retLine=ax.scatter(scaColorC[1],scaColorS[1],s=5,color='black',marker='v',label='electron')
        handles,labels=ax.get_legend_handles_labels()
        ax.legend(handles,labels,loc='upper right',fontsize='small')
        scalarMap.set_array(Erange)
        cbar=fig.colorbar(scalarMap)
    ...
    if(gv.jetactivated==0):
        ...
        plt.savefig('imgs/'+figname,dpi=res) #the line below makes figure with the parameters already written in the title
...
def plotevs(figname,ev,Morp=False,circ=False):
    tcospsol=np.array([[ev[j][i][0]*np.cos(ev[j][i][1])*1/gv.degrad for i in range(len(ev[j]))] for j in range(len(ev))])
    tsinpsol=np.array([[ev[j][i][0]*np.sin(ev[j][i][1])*1/gv.degrad for i in range(len(ev[j]))] for j in range(len(ev))])
    Ebins=gv.EBins
    hsv = plt.get_cmap('hsv')
    ...
    for i in range(len(Ebins)-1):
        retLine=ax.scatter(tcospsol[i],tsinpsol[i],s=2,color=scalarMap.to_rgba(i),label='%.1f'%Ebins[i]+'-'+'%.1f'%Ebins[i+1]+' GeV') 
...
def transportcoordsys(angle):
    ...
    y2=np.cos(angle[0])*np.sin(angle[1])*tpara+np.sin(angle[0])*co.zhat-np.cos(angle[1])*tperp
    x2=np.cos(angle[0])*np.cos(angle[1])*tpara+np.sin(angle[0])*co.zhat+np.sin(angle[1])*tperp

    return x2,y2,z2

def setupB(blazlocs,ds):
    gv.blazarloc=np.array([np.sin(blazlocs[0])*np.cos(blazlocs[1]),np.sin(blazlocs[0])*np.sin(blazlocs[1]),np.cos(blazlocs[0])])*ds
...
def allskyblazarsevents(numblazars):
    blazlocs=np.concatenate([blazarlocs(numblazars[0],1.0,0),blazarlocs(numblazars[1],-1.0,0)])
    gammaevents=[]
    blazloc2=[]
    for j in range(len(blazlocs)):
        ...
        setupB(blazlocs[j],ds) #sets up the magnetic field in the correct orientation.
        
        NumPhotons=gv.drawNumPhotons(ds,gv.flux,1.0)
        tcospsol,tsinpsol,events=stomorp(ds,NumPhotons,alpha=alpha,phi=phi,omega=omega) #create the events.
        for k in range(len(tcospsol)):
            b=(blazlocs[j][0]+tsinpsol[k])%(360.)
            l=(blazlocs[j][1]+tcospsol[k])%(360.)
            if(b>180): 
                b=180-(b-180)
                l=(l+180)%360
...
def morpofblazar(blazlocs2):
    ...
    for j in range(len(blazlocs2)):
        ...
        for k in range(dotsnum):
            b=(blazlocs2[j][0]+ts[0][k])%(360.)
            l=(blazlocs2[j][1]+tc[0][k])%(360.)
            if(b>180): 
                b=180-(b-180)
                l=(l+180)%360
            blazmorpB.append(b)
            blazmorpL.append(l)
            b=(blazlocs2[j][0]+ts[1][k])%(360.)
            l=(blazlocs2[j][1]+tc[1][k])%(360.)
            if(b>180): 
                b=180-(b-180)
                l=(l+180)%360
            blazmorpB.append(b)
