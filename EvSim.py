...
if not sys.version_info[0] < 3:
    from importlib import (
        reload,
    )  # reload function is from a module for python 3 and higher
...
def createplot3DEvs(cn, i, jet=False, sizemarker=5):
    ...
    if jet == True:
        jetData = [
            np.loadtxt("sim_data/3devents/case" + str(cn) + "/jetcart_" + str(i + 1))
            for i in range(njets)
        ]
        jetDataPlot = np.array(
            [
                [
                    [jetData[i][j][0] for j in range(len(jetData[i]))],
...
def JetEvs(NumJets=10, omega=5 * gv.degrad, cn=1, doplots=False, jetfix=True):
    ...
    while jetcount < NumJets:
        if jetfix:
            ...
            jetSel = [jth, jph]
            jetSelCart = [
                np.sin(jth) * np.cos(jph + np.pi),
                np.sin(jth) * np.sin(jph + np.pi),
                np.cos(jth),
            ]
        else:
        ...
        for i in range(len(evsangle)):
            cosang = np.absolute(
                gv.angledotprod(jetSel, [evsangle[i][0], evsangle[i][1]])
            )
            if (
                np.cos(omega) > cosang
        ...
        if len(jetevs) > 2 or attempts == 20:
            ...
            plt.tight_layout(pad=0.6)
            plt.savefig(
                "sim_data/3devents/case" + str(cn) + "/jetskyimg_" + str(jetcount),
                dpi=res,
            )
            plt.close()

            np.savetxt(
                "sim_data/3devents/case" + str(cn) + "/jetopenning_" + str(jetcount),
                [omega],
            )
            np.savetxt(
                "sim_data/3devents/case" + str(cn) + "/jetdir_" + str(jetcount),
                jetSelCart,
            )
            np.savetxt(
                "sim_data/3devents/case" + str(cn) + "/jetcart_" + str(jetcount), jetevs
            )
            np.savetxt(
                "sim_data/3devents/case" + str(cn) + "/jetang_" + str(jetcount), jetang
            )
    ...
    for jetcount in range(NumJets):
        ...
        if UseUIBins:
            ...
            for nbins in range(len(Ebins) - 1):
                evs[jetcount].append(
                    evstemp[np.logical_and(En > Ebins[nbins], En < Ebins[nbins + 1])]
                )
            ...
            for j in range(1, NumEBins):
                evs[jetcount].append(
                    evstemp[ind[extra + bins * j : extra + bins * (j + 1)]]
                )
                Ebins.append(j + 1)
        ...
        Q.append(
            qs.analyzeq(evs[jetcount], blazarloc=True, CQ=CQ)[0]
        )  # [0] chooses Qtot (analyzeq returns 3 different Q's)

        for p in range(len(Q[jetcount])):
            np.savetxt(
                "sim_data/3devents/case"
                + str(cn)
                + "/"
                + Qstart
                + str(jetcount + 1)
        ...
        np.savetxt(
            "sim_data/3devents/case" + str(cn) + "/region_" + str(jetcount + 1),
            gv.region,
        )
...
def QJetsStatistics(NumJets=10, cn=1):
    ...
    combe12 = len(
        [
            ...
            if filename.startswith("sim_data/3devents/case" + str(cn) + "/Q_" + str(1))
        ]
    )  # this might not work
    Qavgjets = [
        [[0 for i in range(angnum)] for j in range(combe12)],
        [[0 for i in range(angnum)] for j in range(combe12)],
    ]
    ...
    for jetcount in range(NumJets):
        prefixed = [
            ...
            if filename.startswith("Q_" + str(jetcount + 1))
        ]
        ...
        for binN in range(len(Qjets[jetcount])):
            for angN in range(len(Qjets[jetcount][binN])):
                Qavgjets[0][binN][angN] = (
                    Qavgjets[0][binN][angN] + Qjets[jetcount][binN][0][angN]
                )
    ...
    for jetcount in range(NumJets):
        for binN in range(len(Qavgjets[0])):
            for angN in range(len(Qavgjets[0][binN])):
                Qavgjets[1][binN][angN] = Qavgjets[1][binN][angN] + (
                    Qjets[jetcount][binN][0][angN] - Qavgjets[0][binN][angN]
                ) * (Qjets[jetcount][binN][0][angN] - Qavgjets[0][binN][angN])

    for binN in range(len(Qavgjets[0])):
        for angN in range(len(Qavgjets[0][binN])):
            Qavgjets[1][binN][angN] = np.sqrt(Qavgjets[1][binN][angN]) / (
                1.0 * (NumJets - 1)
            )
...
def SimNHalos(
    ...
    for i in range(N):
        if (
            case
        ):  # chooses the magnetic field. If case=number, set the field to be that case. Otherwise we use mag field created from Monte Carlo draws or a "uniform"(labeled uni) one.
...
def QsPlot(
    ...
    ThreeHels = type(evsSP) != type(
        False
    )  # do np.shape array, if false. then evsSP must be plotted over the same graph.
    s = (
        screwupfactor
    )  # coord system handness screw up factor. (1 -> -1 to change the handedness)
    ...
    combe12 = len(
        [
            filename
            for filename in os.listdir(directory)
            if filename.startswith(Qstart + "1_bin")
        ]
    )
    ...
    if (
        ...
        Qextratot = [
            [[np.zeros(angnum), np.zeros(angnum)] for j in range(combe12)],
            [[np.zeros(angnum), np.zeros(angnum)] for j in range(combe12)],
        ]
        for INT in range(2):
            for i in evsSP[INT]:
                ...
                Qfiles = [
                    [
                        ...
                        for filename in os.listdir(directory)
                        if np.logical_and(
                            filename.startswith(Qstart),
                            filename.endswith("bin_" + str(k + 1)),
                        )
                    ]
                    for k in range(combe12)
                ]  # Qfiles[i] contains the Q values for the Ebin i+1 for all the jets.
                for j in range(combe12):
                    for qfn in Qfiles[j]:
                        temp = np.loadtxt(qfn)
                        if not np.array_equal(
                            temp, Qtot[0][0]
                        ):  # averaging over the Q's that deviates from 0 only.
                            Qextra[INT][j].append(temp)
                for k in range(len(Qextra[INT])):  # loop over bins
                    if len(Qextra[INT][k]) > 0:
                        temp = np.array(
                            [Qextra[INT][k][i] for i in range(len(Qextra[INT][k]))]
                        )
                    else:
                    ...
                    if len(temp) <= 1:
                        ...
                        Qextratot[INT][k][1] = np.std(temp, axis=0) / np.sqrt(
                            len(temp) - 1
                        )
    ...
    for i in evsnum:
        ...
        Qfiles = [
            [
                ...
                for filename in os.listdir(directory)
                if np.logical_and(
                    filename.startswith(Qstart), filename.endswith("bin_" + str(k + 1))
                )
            ]
            for k in range(combe12)
        ]  # Qfiles[i] contains the Q values for the Ebin i+1 for all the jets.
    ...
    if (
        np.loadtxt("sim_data/3devents/case" + str(evsnum[0]) + "/BinsSelection") == 0
    ):  # This uses UI bins
        plotlabels = [
            str(Ebins[e12comb[iloop][0]])
            + "-"
            + str(Ebins[e12comb[iloop][0] + 1])
            + " GeV \n"
            + str(Ebins[e12comb[iloop][1]])
            + "-"
            + str(Ebins[e12comb[iloop][1] + 1])
            + " GeV"
            for iloop in range(combe12)
        ]
    else:  # Likely use AutoBins --- Unless more options were added.
        plotlabels = [
            "Bins "
            + str(int(Ebins[e12comb[iloop][0]]))
            + "&"
            + str(int(Ebins[e12comb[iloop][1]]))
            for iloop in range(combe12)
        ]
    ...
    if ThreeHels:
        ...
        black_line = mlines.Line2D(
            [], [], marker="None", color=col1, label=labc1, ls="dashed"
        )
        blue_line = mlines.Line2D(
            [], [], marker="None", color=col2, label=labc2, ls="dotted"
        )

        plt.legend(
            handles=[red_line, black_line, blue_line],
    ...
    for k in range(combe12):
        ...
        if ThreeHels:
            ...
            temp.fill_between(
                region,
                s * Qextratot[0][k][0] - Qextratot[0][k][1],
                s * Qextratot[0][k][0] + Qextratot[0][k][1],
                alpha=0.5,
                facecolor=col1,
            )
            ...
            temp.fill_between(
                region,
                s * Qextratot[1][k][0] - Qextratot[1][k][1],
                s * Qextratot[1][k][0] + Qextratot[1][k][1],
                alpha=0.5,
                facecolor=col2,
            )
        ...
        if k < combe12 - 1:
            temp.tick_params(
                axis="x", which="both", labelbottom="off", labelsize=labSize
            )
...
def showfig(
    ...
    nplots = 1
    images = [
        "sim_data/3devents/case" + str(n) + "/jetskyimg_" + str(i + 1) + ".png"
        for i in range(njets)
    ]
    ...
    if show3dplots:
        ...
        if (not os.path.exists(imgs3dpath)) or replot3D:
            for i in range(njets):
                createplot3DEvs(n, i)
        imgs3dpath = [
            "sim_data/3devents/case" + str(n) + "/3dimg_" + str(i + 1) + ".png"
            for i in range(njets)
        ]
    ...
    if phHel:
        nplots = 4
        imageshel = [
            "sim_data/3devents/case" + str(n) + "/BcB.png" for i in range(njets)
        ]
