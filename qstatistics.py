...
def genevents(ntsr,NumPhotons,ds=gv.dS,phi=0,omega=np.pi,alpha=0,jetact=0,SBL=np.array([0,0]),RMF_K=np.array([0,0,0])):
    gv.jetactivated=0
    if(omega<np.pi):
        gv.jetactivated=1
    if(SBL[0]!=0 or SBL[1]!=0): #SBL is the location of the blazar in radiants (b,l), (0,0) is the north pole
        gv.fullsky=True
    ...
    a1,a2,evtemp=sM.stomorp(ds,NumPhotons,alpha=alpha,phi=phi,omega=omega) #events from the blazar
    evtemp=np.array(evtemp)
    EnBins=gv.EBins #determining the Ebins.   
    ev=[evtemp[(evtemp[:,2]>EnBins[j])&(evtemp[:,2]<EnBins[j+1])] for j in range(len(EnBins)-1)]
    ev=[ev[j][:,0:2] for j in range(len(ev))] #events are binned per Energy and have the form [th,phi]
    ...
    evbacktemp=np.array(mcb.SBbackground(NumPhotons,EnBins,ntsr,thcut,thcutHE))*gv.degrad#Events of the background in radians
    #print('MC Events per Ebins ',[len(evbacktemp[i]) for i in range(len(evbacktemp))])
    evtot=np.array([np.concatenate((ev[j],evbacktemp[j]),axis=0) for j in range(len(EnBins)-1)])
    evC=np.array([np.array([gv.angulartocart(ev[j][i][0],ev[j][i][1]) for i in range(len(ev[j]))]) for j in range(len(EnBins)-1)])
    evbacktempC=np.array([np.array([gv.angulartocart(evbacktemp[j][i][0],evbacktemp[j][i][1]) for i in range(len(evbacktemp[j]))]) for j in range(len(EnBins)-1)])
    evtotC=np.array([np.array([gv.angulartocart(evtot[j][i][0],evtot[j][i][1]) for i in range(len(evtot[j]))]) for j in range(len(EnBins)-1)])
...
def analyzeq(evtot,evBl=False,evMc=False,blazarloc=False,CQ=0):
    ...
    if(blazarloc):
        nume12=len(evtot)
        combe12 = int(np.math.factorial(nume12)/np.math.factorial(nume12 - 2) /np.math.factorial(2))
...
def qstats(numruns,ntsr,NumPhotons,totalonly=False,jetact=0):
    ...
    combe12 = int(np.math.factorial(nume12)/np.math.factorial(nume12 - 2) /np.math.factorial(2))
    Qtot,Qbla,Qmc=np.zeros((numruns,combe12,2,len(gv.region))),np.zeros((numruns,combe12,2,len(gv.region))),np.zeros((numruns,combe12,2,len(gv.region)))
    if(totalonly):
        evBlC=False
        evBaC=False
    ...
    QtotA,QblaA,QmcA=np.zeros((combe12,2,len(gv.region))),np.zeros((combe12,2,len(gv.region))),np.zeros((combe12,2,len(gv.region)))
    ...
    for j in range(combe12):
        for p in range(len(gv.region)):
            ...
            for i in range(numruns):
                QtotA[j,1,p]=QtotA[j,1,p]+np.power(Qtot[i,j,0,p]-QtotA[j,0,p],2.0)
                if(not totalonly):
                    QblaA[j,1,p]=QblaA[j,1,p]+np.power(Qbla[i,j,0,p]-QblaA[j,0,p],2.0)
                    QmcA[j,1,p]=QmcA[j,1,p]+np.power(Qmc[i,j,0,p]-QmcA[j,0,p],2.0)
